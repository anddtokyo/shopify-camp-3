...
const METAFIELDS_FULFILLMENT_SERVICE_KEY = "fulfillment_service";
 ...
 * @return {Promise<string|null>}
 */
export async function updateFulfillmentServiceId(session, id) {
  const shop = await getShopFromSession(session);
  const client = new shopify.api.clients.Graphql({ session });
  const response = await client.query({
    data: {
  ...
  `,
      variables: {
        metafields: [
          {
            key: METAFIELDS_FULFILLMENT_SERVICE_KEY,
            namespace: METAFIELDS_NAMESPACE,
            ownerId: shop.id,
            type: "single_line_text_field",
            value: id,
 ...
 * @return {Promise<string|null>}
 */
export async function getShopFulfillmentServiceId(session) {
  const client = new shopify.api.clients.Graphql({ session });
  const response = await client.query({
    data: {
      query: `query {
  shop {
    name
    metafield (namespace: "${METAFIELDS_NAMESPACE}", key: "${METAFIELDS_FULFILLMENT_SERVICE_KEY}") {
      value
    }
  }
}
